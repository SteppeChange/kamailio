...
static void cetcd_node_print(cetcd_response_node *node) {
    ...
    if (node) {
        LM_INFO("Node TTL: %lu\n", node->ttl);
        LM_INFO("Node ModifiedIndex: %lu\n", node->modified_index);
        LM_INFO("Node CreatedIndex: %lu\n", node->created_index);
        LM_INFO("Node Key: %s\n", node->key);
        LM_INFO("Node Value: %s\n", node->value);
        LM_INFO("Node Dir: %d\n", node->dir);
        LM_INFO("\n");
        if (node->nodes) {
            count = cetcd_array_size(node->nodes);
...
void cetcd_response_print(cetcd_response *resp) {
    if (resp->err) {
        LM_ERR("Error Code:%d\n", resp->err->ecode);
        LM_ERR("Error Message:%s\n", resp->err->message);
        LM_ERR("Error Cause:%s\n", resp->err->cause);
        return;
    }
    LM_INFO("Etcd Action:%s\n", cetcd_event_action[resp->action]);
    LM_INFO("Etcd Index:%lu\n", resp->etcd_index);
    LM_INFO("Raft Index:%lu\n", resp->raft_index);
    LM_INFO("Raft Term:%lu\n", resp->raft_term);
    if (resp->node) {
        LM_INFO("-------------Node------------\n");
